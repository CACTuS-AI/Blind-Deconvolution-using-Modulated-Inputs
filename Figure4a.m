...
Q = 800;%length of signals
L = 3200; %length of ffts L >= max(Q,M)
J = 10; % Number of frames
it = 20;
success_freq = zeros(it,it);
for k = 1:1:it
    M = k*40; % length of h
    ehP = zeros(it,J);
    for IT = 1:1:it
        K = IT*40; %length of signal expansion coefficents
        for num = 1:J
            %% Synthetic Signals 
